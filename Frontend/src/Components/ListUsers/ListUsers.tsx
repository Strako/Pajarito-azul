...
const listUsers = ({ keyToUpdate, usersArray, navigate }: propsI) => {
    if (usersArray) {
        const users = usersArray.map((user: objectI) => (
            <>
                <div key={user.userId} id={user.userId} className="user_container" >
                    ...
                    <div className="user_followers">20 Followers</div>
        ...
        return (
            <>
                <div className="user_container">
                    <Empty    image="https://gw.alipayobjects.com/zos/antfincdn/ZHrcdLPrvN/empty.svg" description={<span style={{ color: 'white'   }}>Theres no users found :c </span>}/>
                </div>
            </>
        )
