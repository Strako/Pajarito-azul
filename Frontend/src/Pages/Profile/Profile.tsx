...
const Profile = () => {
  ...
  const handleEditTweet: React.ChangeEventHandler<HTMLTextAreaElement> = (
    e
  ) => {
  ...
  const handleProfileImage: React.ChangeEventHandler<HTMLInputElement> = (
    e
  ) => {
    setProfileImage(e.target.value);
  };
  const handleProfileDescription: React.ChangeEventHandler<HTMLInputElement> = (
    e
  ) => {
  ...
  const handleRefresh = () => {
    setTimeout(() => {
      ...
      getTweets(user.user, "1").then((r) => {
        ...
        setListTweetsKey((prevKey) =>
          prevKey === "initialKey" ? "refreshKey" : "initialKey"
        );
      });
    }, 500);
  };
  ...
  return (
    <>
      ...
      >
        <Typography.Title level={5} style={{ fontSize: "16px" }}>
          Username
        </Typography.Title>
        ...
        {inputOneLine("20px", 1000, handleProfileImage, profileImage)}
        <Typography.Title level={5} style={{ fontSize: "16px" }}>
          Description
        </Typography.Title>
        {inputOneLine(
