...
const SingleTweet = () => {
    ...
    setTimeout(() => {
      getCommentsByID(tweetID, 1).then((r) => {
        ...
        setCommentsArray((prevComments) => [
          r.data.comments[commentsIds[0]],
          ...prevComments,
        ]);
        console.log({ HandlreRefresh: commentsArray });
      });
    }, 200);
    //window.location.reload();
  };

  const handleEditComment: React.ChangeEventHandler<HTMLTextAreaElement> = (
    e
  ) => {
  ...
  const handleOk = () => {
    ...
    setTimeout(() => {
      ...
      console.log({ handleOK: commentsNumber });
    }, 200);
  };
      ...
      getCommentsByID(tweetID, page).then((r) => {
        if (r.data.empty === true) {
          ...
          for (let i = 0; i < commentsIDs.length; i++) {
            setCommentsArray((oldArray) => [
              ...oldArray,
              r.data.comments[commentsIDs[i]],
            ]);
          }
          setIsLoading(false);
        }
    ...
    commentsArray.map((comment: objectI) => {
      if (mapComments.has(comment.userID) === false) {
        getUserByID(comment.userID).then((r) => {
          setMapcomments((prevMapComments) =>
            new Map(prevMapComments).set(comment.userID, {
              user: r.data.user,
              image: r.data.userImage,
            })
          );
        });
        console.log({ "mapComments in useEffect": mapComments });
      }
      console.log({ "NoFOR mapComments in useEffect": mapComments });
    });
  }, [commentsArray, commentsNumber]);
  ...
  const listTweet = () => {
    ...
    const tweet = (
      <>
        {" "}
        <div
          key={tweetContent.tweetID}
          id={tweetContent.tweetID}
          className="single_tweet"
        >
          <img
            className="single_tweet_img"
            src={user.userImage}
            loading="lazy"
            onClick={() => navigate("/user/" + user.user)}
          ></img>
          <div
            className="single_tweet_author"
            onClick={() => navigate("/user/" + user.user)}
          >
            {" "}
            {user.user}
          ...
          <div className="single_tweet_content">
            ...
            <div className="single_comment_icon" onClick={showModal}>
              <CommentOutlined />{" "}
            </div>
  ...
  return (
    <>
      <SidebarTemplate>
        <div className="main">
          <div className="single_tweets_container">{listTweet()}</div>
